...
export default function ActorDetailScreen() {
  ...
  const [aktor, setAktor] = useState<AktorDetail | null>(null);
  ...
  return (
    <View style={styles.page}>
      <Stack.Screen options={{ title: aktor?.name ?? "Detail Aktor" }} />
      <NavigationBar />
      <ScrollView contentContainerStyle={styles.container}>
        <Link href="/aktor" style={styles.backLink}>
          <Text style={styles.backText}>← Kembali ke daftar Aktor</Text>
        </Link>
        ...
        {isLoading ? (
          ...
          <>
            <View style={[styles.hero, isMediumScreen && styles.heroDesktop]}>
              <Image
                source={{ uri: `${API_URL}${aktor.image}` }}
              ...
              <View style={[styles.heroBody, isMediumScreen && styles.heroBodyDesktop]}>
                <Text style={styles.title}>{aktor.name}</Text>
                <Text style={styles.meta}>Aktor</Text>
                <Text style={styles.meta}>Tanggal Lahir: {new Date(aktor.tanggal_lahir).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}</Text>
                <Text style={styles.meta}>Asal: {aktor.tempat_asal}</Text>
                <Text style={styles.description}>{aktor.description}</Text>
              </View>
            </View>

            <Text style={styles.sectionTitle}>Film yang Dibintangi</Text>
            {aktor.film.length ? (
          ...
          <Text style={styles.emptyText}>Aktor tidak ditemukan.</Text>
        )}
      </ScrollView>
    </View>
  );
